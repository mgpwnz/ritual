...
sudo apt update -y
sudo apt install -y snap mc wget curl git htop netcat-openbsd net-tools \
    unzip jq build-essential ncdu tmux make cmake clang pkg-config \
    libssl-dev protobuf-compiler bc lz4 screen
...
if ! docker compose version &> /dev/null; then
    docker_compose_version=$(wget -qO- \
...
sed -i 's|address registry = .*|address registry = '"$REGISTRY_ADDRESS"';|' \
    projects/hello-world/contracts/script/Deploy.s.sol
sed -i 's|sender := .*|sender := '"$PRIVATE_KEY"'|' \
    projects/hello-world/contracts/Makefile
sed -i 's|RPC_URL := .*|RPC_URL := '"$RPC_URL"'|' \
    projects/hello-world/contracts/Makefile
...
# === Ensure no conflicting /usr/bin/forge ===
...
sed -i 's|0x13D69Cf7d6CE4218F646B759Dcf334D82c023d8e|'"$CONTRACT_ADDRESS"'|' \
    "projects/hello-world/contracts/script/CallContract.s.sol"

# === Call Contract ===
project=hello-world make call-contract

# === Final Docker Compose for Node ===
cd deploy
docker compose down
sleep 3
